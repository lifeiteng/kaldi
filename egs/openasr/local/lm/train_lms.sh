...
if [ $# -ne 2 ]; then
  echo "Usage: local/lm/train_lms.sh <train-text> <test-text>"
  echo " Options:"
...
[ ! -f $srcdir/lexicon.txt ] && echo "Expect $srcdir/lexicon.txt exist!" && exit 1;
...
if [ ! -f $srcdir/lexicon.txt ]; then
  echo "Expect $srcdir/lexicon.txt to exist";
  echo "You need to run local/std_prepare_dict.sh before running this script."
  exit 1;
fi

# Get a wordlist-- keep everything but silence, which should not appear in
# the LM.
awk '{print $1}' $srcdir/lexicon.txt | grep -v -w '!SIL' > $dir/wordlist.txt || exit 1;

# Get training data with OOV words (w.r.t. our current vocab) replaced with <UNK>.
echo "Getting training data with OOV words replaced with <UNK> (train_nounk.gz)" 

cat $train_text | awk -v w=$dir/wordlist.txt \
...
# 3-gram:
ngram-count -text $train_text -order 3 -limit-vocab -vocab $sdir/wordlist.final.s -unk \
  -map-unk "<UNK>" -kndiscount -interpolate -lm $sdir/srilm.o3g.kn.gz
ngram -lm $sdir/srilm.o3g.kn.gz -ppl $test_text # consider -debug 2

# Trying 4-gram:
ngram-count -text $train_text -order 4 -limit-vocab -vocab $sdir/wordlist.final.s -unk \
  -map-unk "<UNK>" -kndiscount -interpolate -lm $sdir/srilm.o4g.kn.gz
ngram -order 4 -lm $sdir/srilm.o4g.kn.gz -ppl $test_text

#3-gram with pruning:
ngram-count -text $train_text -order 3 -limit-vocab -vocab $sdir/wordlist.final.s -unk \
  -prune 0.0000001 -map-unk "<UNK>" -kndiscount -interpolate -lm $sdir/srilm.o3g.pr7.kn.gz
ngram -lm $sdir/srilm.o3g.pr7.kn.gz -ppl $test_text
...
sdir=$dir/srilm # in case we want to use SRILM to double-check perplexities.
mkdir -p $sdir
gunzip -c $srcdir/cleaned.gz | head -$heldout_sent > $test_text
gunzip -c $srcdir/cleaned.gz | tail -n +$heldout_sent > $train_text
(echo "<s>"; echo "</s>" ) | cat - $dir/wordlist.txt > $sdir/wordlist.final.s
...
ngram -lm $idir/lm_3gram.gz  -ppl $test_text 

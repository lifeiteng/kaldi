...
# Copyright 2014 Feiteng Li
# 参考wsj_format_data.sh
# 2015.01.14
...
lexicon=$data/decode/dict/lexicon.txt
mkdir -p $tmpdir
...
for lm_suffix in $lm_suffixs ; do
  test=$data/lang_decode_${lm_suffix}
  mkdir -p $test
  for f in phones.txt words.txt phones.txt L.fst L_disambig.fst \
     phones/ topo oov.int oov.txt; do
    cp -r $data/decode_lang/$f $test
  done
  cat $lmdir/lm_${lm_suffix}.arpa | \
  ...
  rm -r $tmpdir/g
  utils/validate_lang.pl $data/lang_test_${lm_suffix}|| exit 1
done

...
if [ $stage -le -1 ]; then
  ...
  if ${remove_precondition};then
    $cmd $dir/log/convert.log \
      nnet-am-copy --remove-preconditioning=true --learning-rate=$learning_rate "$src_model" $dir/0.mdl || exit 1;
  else
    $cmd $dir/log/convert.log \
      nnet-am-copy --remove-preconditioning=${remove_precondition} --learning-rate=$learning_rate "$src_model" - \| \
...
while [ $x -lt $num_iters ]; do
  if [ $stage -le $x ]; then
    ...
    if $multigpu && [ $gpunum -gt 1 ]; then
      echo "Multi-GPU($gpunum GPUs) NNet2 Discriminative Training($num_jobs_nnet nnet jobs)... "
      ( 
        n=1
        ...
        while [ $n -le $num_jobs_nnet ]; do
          for gpuid in $(seq $gpunum); do
            if [ $n -le $num_jobs_nnet ]; then 
              $cmd $dir/log/train.$x.$n.log \
                nnet-combine-egs-discriminative \
                  "ark:$degs_dir/degs.\$[(($n-1+($x*$num_jobs_nnet))%$num_archives)+1].ark" ark:- \| \
                nnet-train-discriminative$train_suffix --silence-phones=$silphonelist \
                 ...
                 --gpu-id=$[$gpuid-1] $dir/$x.mdl ark:- $dir/$[$x+1].$n.mdl || touch $dir/$[$x+1].$n.error &
      ...
      echo "Single-GPU($gpunum GPUs) NNet2 Discriminative Training($num_jobs_nnet nnet jobs)... "
      (
        for n in $(seq $num_jobs_nnet); do
          $cmd $dir/log/train.$x.$n.log \
            ...
            nnet-train-discriminative$train_suffix --silence-phones=$silphonelist \
             ...
             --one-silence-class=$one_silence_class \
             --boost=$boost --acoustic-scale=$acoustic_scale \
             $dir/$x.mdl ark:- $dir/$[$x+1].$n.mdl || touch $dir/$[$x+1].$n.error
        done
      )
    fi      

...
# Feiteng 2014.12.26 2014.12.27 
# 必须指定nnet2模型文件，进行 fine-tuning
# 2015.01.22 add dropout 支持
...
add_dropout=false
add_dropout_epoch=0
dropout_layers="" #3:5:9
dropout_proportions=""
#dropout_scale=0.0

remove_dropout=false
remove_dropout_epoch=4
passwd="496666"
show_progress=true
weight_sil=false

silence_weight=1 # Weight on silence
...
prior_subset_size=100000 # 10k samples per job, for computing priors.  Should be
                        # more than enough.
...
cmvn_opts="--norm-means=false --norm-vars=false"  
            # will be passed to get_lda.sh and get_egs.sh, if supplied.  
...
if [ $stage -le -4 ]; then 
  if [ -z "$nnet_file" ]; then
    echo "必须指定nnet模型文件,退出." && exit 1;
  else
    echo "copy $nnet_file -> $dir/0.mdl "
    cp $nnet_file $dir/0.mdl
  fi
fi
...
# Check some files.
for f in $data/feats.scp $lang/L.fst $alidir/ali.1.gz $alidir/tree; do
...
if [ $num_leaves -eq 0 ];then
  echo "\$num_leaves is 0" && exit 1
fi
...
# if [ $stage -le -4 ]; then ## 这里的LDA包含了 -mean /var
#   echo "$0: calling get_lda.sh"  ## 包含 echo $cmvn_opts >$dir/cmvn_opts 2>/dev/null
...
if [ $stage -le -3 ] && [ -z "$egs_dir" ]; then
  ...
  if ${weight_sil};then
    echo "$0: weight silence posterior(${silence_weight})"
    std/local/nnet2/get_egs2_weight_sil.sh $egs_opts "${extra_opts[@]}"  --io-opts "$io_opts" \
      --samples-per-iter $samples_per_iter --stage $get_egs_stage \
      --cmd "$cmd" --silence-weight ${silence_weight} $egs_opts $data $lang $alidir $dir/egs || exit 1;
  else
...
if [ $stage -le -1 ] && [ ! $fine_tuning ]; then
    echo "Training transition probabilities and setting priors(silence-weight=${silence_weight})"
    $cmd $dir/log/train_trans.log \
      nnet-train-transitions --silence-weight=${silence_weight} --silence-phones-str=$silphonelist $dir/0.mdl "ark:gunzip -c $alidir/ali.*.gz|" $dir/0.mdl \
      || exit 1;
fi
...
add_dropout_iter=-100
if $add_dropout &&  [ $add_dropout_epoch -ge 0 ] ; then
  add_dropout_iter=$(perl -e 'print int(($ARGV[0]*$ARGV[1])/$ARGV[2]);' $add_dropout_epoch $num_archives_expanded $num_jobs_nnet)
  echo "will add dropout components at iter $add_dropout_iter, at layers--$dropout_layers."
fi

remove_dropout_iter=-100
if $remove_dropout &&  [ $remove_dropout_epoch -ge 0 ] ; then
  remove_dropout_iter=$(perl -e 'print int(($ARGV[0]*$ARGV[1])/$ARGV[2]);' $remove_dropout_epoch $num_archives_expanded $num_jobs_nnet)
  echo "will remove dropout components at iter $remove_dropout_iter."
fi
...
while [ $x -lt $num_iters ]; do
  ...
  if [ $x -ge 0 ] && [ $stage -le $x ]; then
    ...
    if [ $x -eq $add_dropout_iter ] && $add_dropout ; then
      echo "adding dropout components..."
      $cmd $dir/log/add_dropout.$x.log \
        nnet-am-copy --add-dropout=true --dropout-layers=$dropout_layers \
          --dropout-proportions=$dropout_proportions $dir/$x.mdl $dir/$x.mdl || exit 1;
    fi

    if [ $x -eq $remove_dropout_iter ] && $remove_dropout ; then
      echo "removing dropout components..."
      $cmd $dir/log/remove_dropout.$x.log \
        nnet-am-copy --remove-dropout=true $dir/$x.mdl $dir/$x.mdl || exit 1;
    fi
    ...
    if [ ! -z "${realign_this_iter[$x]}" ]; then
      ...
      steps/nnet2/align.sh --nj $num_jobs_align --cmd "$align_cmd" --use-gpu $align_use_gpu \
        ...
        --gpunum $gpunum --iter $x $data $lang $dir $dir/ali_$epoch || exit 1
    ...
    if [ $x -ge $add_dropout_iter ] && [ $x -le $remove_dropout_iter ]; then
      mdl="nnet-am-copy --remove-dropout=true $dir/$x.mdl - |"
      $cmd $dir/log/compute_prob_valid.$x.log \
        nnet-compute-prob "$mdl" ark:$cur_egs_dir/valid_diagnostic.egs &
      $cmd $dir/log/compute_prob_train.$x.log \
        nnet-compute-prob "$mdl" ark:$cur_egs_dir/train_diagnostic.egs &
    else
      $cmd $dir/log/compute_prob_valid.$x.log \
    ...
    if $show_progress && [ $x -gt 0 ] && [ ! -f $dir/log/mix_up.$[$x-1].log ]; then
    ...
    if $multigpu && [ $gpunum -gt 1 ]; then
      echo "Multi-GPU($gpunum GPUs) NNet2 Training($num_jobs_nnet nnet jobs)... "
      ( # this sub-shell is so that when we "wait" below,
        ...
        n=1
        while [ $n -le $num_jobs_nnet ]; do
          for gpuid in $(seq $gpunum); do
            if [ $n -le $num_jobs_nnet ]; then            
              k=$[$x*$num_jobs_nnet + $n - 1]; # k is a zero-based index that we'll derive
                                               # the other indexes from.
              archive=$[($k%$num_archives)+1]; # work out the 1-based archive index.
              frame=$[((($k/$num_archives)%$frames_per_eg))]; # add ()
              # work out the 0-based frame
              ...
              $cmd $parallel_opts $dir/log/train.$x.$n.log \
                nnet-train-simple $parallel_train_opts \
                --gpu-id=$[$gpuid-1] --minibatch-size=$this_minibatch_size --srand=$x "$mdl" \
                ...
                $dir/$[$x+1].$n.mdl || touch $dir/$[$x+1].$n.error &
      ...
      echo "Single-GPU($gpunum GPUs) NNet2 Training($num_jobs_nnet nnet jobs)... "
      ( # this sub-shell is so that when we "wait" below,
        # we only wait for the training jobs that we just spawned,
        # not the diagnostic jobs that we spawned above.
    ...
    for n in `seq 1 $num_jobs_nnet`; do
      [ -f $dir/$[$x+1].$n.error ] && echo "$0: error on iteration $x . $n of training" && continue;
      nnets_list="$nnets_list $dir/$[$x+1].$n.mdl"
    done

    if [ -z "${nnets_list}" ];then
      echo "$0: error on iteration $x of training(all jobs failed.), $dir/$[$x].mdl $dir/$[$x+1].mdl";
      cp $dir/$[$x].mdl $dir/$[$x+1].mdl
      x=$[$x+1]
      continue
    fi
    ...
    if [ -f $dir/$[$x-1].mdl ] && $cleanup && \
       [ $[($x-1)%50] -ne 0  ] && [ $[$x-1] -lt $first_model_combine ]; then
      rm $dir/$[$x-1].mdl
    fi
  fi
  x=$[$x+1]
...
if $do_combination && [ $stage -le $[$num_iters+1] ]; then
  ...
  # # pnorm layer and then a normalize layer. ->Sigmoid Layer (Feiteng)
...
if [ ! -f $dir/final.mdl ]; then
	cp $dir/$[$num_iters-1].mdl $dir/final.mdl 
fi
...
if [ $stage -le $[$num_iters+1] ]; then
  ...
  rm $dir/post.$x.*.vec 2>/dev/null
  $cmd JOB=1:$num_jobs_compute_prior $dir/log/get_post.$x.JOB.log \
    nnet-copy-egs --frame=random --srand=JOB ark:$cur_egs_dir/egs.JOB.ark ark:- \| \
    nnet-subset-egs --srand=JOB --n=$prior_subset_size ark:- ark:- \| \
  ...
  $cmd $dir/log/vector_sum.$x.log \
   vector-sum --binary=false $dir/post.$x.*.vec $dir/post.$x.vec || exit 1;
...
if $cleanup; then
  ...
  if $cleanupegs && [[ $cur_egs_dir =~ $dir/egs* ]]; then

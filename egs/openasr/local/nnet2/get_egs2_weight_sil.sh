...
if [ $# != 4 ]; then
  echo "Usage: $0 [opts] <data> <lang> <ali-dir> <egs-dir>"
...
if [ $stage -le 2 ]; then
  ...
  $cmd $dir/log/create_valid_subset.log \
    nnet-get-egs $ivectors_opt $nnet_context_opts "$valid_feats" \
      "ark,s,cs:gunzip -c $dir/ali_special.gz | ali-to-pdf $alidir/final.mdl ark:- ark:- | ali-to-post ark:- ark:- | weight-silence-post --pdf=true $silence_weight $silphonelist $alidir/final.mdl ark:- ark:- |" \
...
if [ $stage -le 3 ]; then
  ...
  if [ ! -z $postdir ]; then    
    for n in $(seq $nj); do
      $cmd $io_opts JOB=$n:$n $dir/log/get_egs.JOB.log \
    ...
    for n in $(seq $nj); do
      $cmd $io_opts JOB=$n:$n $dir/log/get_egs.JOB.log \
        nnet-get-egs $ivectors_opt $nnet_context_opts --num-frames=$frames_per_eg "$feats" \
          "ark:gunzip -c $alidir/ali.*.gz | ali-to-pdf $alidir/final.mdl ark:- ark:- | ali-to-post ark:- ark:- | weight-silence-post --pdf=true $silence_weight $silphonelist $alidir/final.mdl ark:- ark:- |" ark:- \| \
          nnet-copy-egs ark:- $egs_list || exit 1;
    done
  ...
  #   $cmd $io_opts JOB=1:$nj $dir/log/get_egs.JOB.log \
  #     nnet-get-egs $ivectors_opt $nnet_context_opts --num-frames=$frames_per_eg "$feats" \
  #     "ark,s,cs:gunzip -c $alidir/ali.JOB.gz | ali-to-pdf $alidir/final.mdl ark:- ark:- | ali-to-post ark:- ark:- | 
  #     weight-silence-post --pdf=true $silence_weight $silphonelist $alidir/final.mdl ark:- ark:- |" ark:- \| \
  #     nnet-copy-egs ark:- $egs_list || exit 1;
...
if [ $stage -le 4 ]; then
  ...
  for n in $(seq $num_archives); do
    $cmd $io_opts $extra_opts JOB=$n:$n $dir/log/shuffle.JOB.log \

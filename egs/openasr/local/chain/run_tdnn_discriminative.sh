...
if [ $frame_subsampling_factor -ne 1 ]; then
  if [ $stage -le 0 ]; then
    ...
    for x in `seq -$[frame_subsampling_factor/2] $[frame_subsampling_factor/2]`; do 
      steps/shift_feats.sh --cmd "$train_cmd --max-jobs-run 12" --nj $nj \
...
if [ $stage -le 11]; then
  ...
  for x in `seq $decode_start_epoch $num_epochs`; do
      iter=epoch$x.adj
      local/decode/online_decode.sh --decode-sets "forum non-native native" --decode-nj 12 --graph-dir $graph_dir \
        --data data_fbank_hires --decode-iter $iter \
        --online-cmvn-opts " --online-cmvn true --cmvn-scp $cmvn_scp " \
        --feat-config "--feature-type mfcc --mfcc-config conf/mfcc_hires.conf"  \
        --decode-suff "_peruser" --chain true --scoring-opts "--min-lmwt 5 " \
        $dir
  done
fi

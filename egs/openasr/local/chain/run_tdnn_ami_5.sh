...
###################################
if [ $stage -le 0 ]; then
  while [ ! -f data_fbank/data_tedlium_fbank/$train_set ];do
    echo "wait : sleep 60"
    sleep 60
  done
  sleep 60

  rm -f $data/cleanup.key
  for x in data_fbank/non_data_fbank/train data_fbank/nat_data_fbank/train data_fbank/data_tedlium_fbank/train;do
    awk '{print $1;}' ${x}_cleanup/text >>$data/cleanup.key
  done
  rm -rf $data/$train_set
  cp -r $data/train $data/$train_set
  cp $data/$train_set/feats.scp $data/$train_set/feats.scp.bk
  awk 'FNR==NR{Z[$1]=1;} FNR<NR{if(($1 in Z)) print $0;}' $data/cleanup.key $data/$train_set/feats.scp.bk >$data/$train_set/feats.scp || exit 1

  for x in `seq 1 6`;do
    cp $data/$train_set/split6/$x/feats.scp $data/$train_set/split6/$x/feats.scp.bk
    awk 'FNR==NR{Z[$1]=1;} FNR<NR{if(($1 in Z)) print $0;}' $data/cleanup.key \
      $data/$train_set/split6/$x/feats.scp.bk >$data/$train_set/split6/$x/feats.scp || exit 1;
  done

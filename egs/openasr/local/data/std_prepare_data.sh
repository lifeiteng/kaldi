...
if [ ! -f $ROOTCORPUS/doc/all_trans.txt ]; then
	if [ ! -f $ROOTCORPUS/doc/transcriptions.all ]; then
		echo "Error: std_prepare_data.sh 语料库路径参数应该包含$ROOTCORPUS/doc/all_trans.txt标注文件."
		exit 1;
	else
		ln -s $ROOTCORPUS/doc/transcriptions.all $ROOTCORPUS/doc/all_trans.txt
	fi
fi
...
if $train;then
	...
	if [ ! -f $ROOTCORPUS/doc/all_trans.txt.back ];then
		...
		local/data/make_trans.py --fix-trans True $ROOTCORPUS/doc/all_trans.txt $ROOTCORPUS/doc/all_trans.txt || exit 1;
	fi
	...
	local/data/make_trans.py --user-filter "$user_filter" --corpus $corpus trn $tmpdir/train.flist $ROOTCORPUS/doc/all_trans.txt >(sort -k1 >$dir/text) \
...
if $test;then
	...
	local/data/make_trans.py --corpus $corpus tst $tmpdir/test.flist $ROOTCORPUS/doc/all_trans.txt >(sort -k1 >$dir/text) \
		>(sort -k1 >$dir/wav.scp) $dir/utt2spk || exit 1;
	sleep 2
	utils/utt2spk_to_spk2utt.pl $dir/utt2spk > $dir/spk2utt || exit 1;
fi

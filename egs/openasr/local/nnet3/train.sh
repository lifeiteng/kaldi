...
num_jobs_compute_prior=6 # these are single-threaded, run on CPU.
...
add_layers_period=10 # by default, add new layers every 2 iterations.
stage=-6
...
align_cmd="run.pl"             # The cmd that is passed to steps/nnet2/align.sh
...
while [ $x -lt $num_iters ]; do
  echo "=============== Start Training Iter: $x on" `date` " ==============="
  ...
  if [ $[$this_num_jobs%3] -eq 1 ];then
    this_num_jobs=$[$this_num_jobs-1]
  fi
  if [ $[$this_num_jobs%3] -eq 2 ];then
    this_num_jobs=$[$this_num_jobs-2]
  fi
  ...
  if [ $x -ge 0 ] && [ $stage -le $x ]; then
    ...
    if [ ! -z "${realign_this_iter[$x]}" ]; then
      ...
      sleep 2
      echo "496666" | sudo -S nvidia-smi -c 0
      local/nnet3/align.sh --nj $num_jobs_align --cmd "$align_cmd" --num-gpus $num_gpus --use-gpu $align_use_gpu \
    ...
    ( # this sub-shell is so that when we "wait" below,
      ...
      echo "Multi-GPU($num_gpus GPUs) NNet3 Training($this_num_jobs nnet jobs): "
        ...
        for gpu_id in $(seq $num_gpus); do       
          if [ $n -le $this_num_jobs ]; then
            ...
            $cmd $train_queue_opt $dir/log/train.$x.$n.log \
              nnet3-train --gpu-id=$[$gpu_id-1] $parallel_train_opts --print-interval=10 --momentum=$momentum \
...
if [ $stage -le $[$num_iters+1] ]; then
  ...
  $cmd JOB=1:$num_jobs_compute_prior $prior_queue_opt $dir/log/get_post.$x.JOB.log \
    ...
    nnet3-compute-from-egs --apply-exp=true \

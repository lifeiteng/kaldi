...
if [ $stage -le 9 ]; then
  ...
  python steps/nnet3/tdnn/make_configs.py  \
    ...
    --ali-dir $ali_dir \
    --relu-dim $relu_dim \
    --bottleneck-dim $hidden_dim \
    --batch-normalize $use_batch_norm \
    --splice-indexes "$splice_indexes"  \
    --use-presoftmax-prior-scale true \
   $dir/configs || exit 1;
fi

[ -z $lang_dir ] && lang_dir=$data/lang
...
if [ $stage -le 10 ]; then
  ...
  exit_opts=""
  [ ! -z $exit_stage ] && exit_opts="--exit-stage ${exit_stage}"
  steps/nnet3/train_dnn.py --stage=$train_stage $exit_opts \
    --cmd="$decode_cmd" --cv-period $cv_period \
...
if ! $skip_decode && [ $stage -le 11 ]; then

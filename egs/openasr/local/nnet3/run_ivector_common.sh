...
if [ "$speed_perturb" == "true" ]; then
  if [ $stage -le 1 ]; then
    ...
    for datadir in $train_sources; do
    ...
    utils/data/combine_data.sh --skip-fix true data/train_sp \
      `echo $train_sources | awk '{for(i=1;i<=NF;i++) {printf "data/"$i"_sp ";} }'` || exit 1;
  fi
...
if [ $stage -le 3 ]; then
  ...
  utils/data/combine_data.sh --skip-fix true data/train_sp_hires \
    `echo $train_sps| awk '{for(i=1;i<=NF;i++) {printf "data/"$i"_hires ";} }'` || exit 1;
fi

if [ $stage -le 4 ]; then
  mfccdir=/ssd/mfcc_hires
  for datadir in spontaneous-forum readaloud-non-native readaloud-native;do
    utils/copy_data_dir.sh data/$datadir data/${datadir}_hires
    steps/make_mfcc.sh --nj $nj --mfcc-config conf/mfcc_hires.conf \
      --cmd "$train_cmd" data/${datadir}_hires exp/make_hires/$datadir $mfccdir || exit 1;
    steps/compute_cmvn_stats.sh data/${datadir}_hires exp/make_hires/$datadir $mfccdir || exit 1;
...
if [ $stage -le 5 ]; then
  train_sps=`echo $train_sources | awk '{for(i=1;i<=NF;i++) {printf $i"_sp ";} }'`
  for datadir in $train_sps;do
    if [ ! -f data/$datadir/feats.scp ];then
      cp data/$datadir/.backup/feats.scp data/$datadir || exit 1;
    fi
  done

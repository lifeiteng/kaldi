...
if [ ! -f $srcdir/lat.1.gz ]; then
  echo "$0: check wether lattices (lat.*.gz) are present in $srcdir."
  exit 1;
fi
...
if [[ -z $utt_prefix && -z $utt_suffix ]];then
  ...
  for i in `seq 1 $num_jobs`; do
      cp $srcdir/lat.$i.gz $destdir/lat.$i.gz || exit 1 &
  done
  wait
else
  ...
  for i in `seq 1 $num_jobs`; do
      lattice-copy --utt-prefix=$utt_prefix --utt-suffix=$utt_suffix \
        "ark:gunzip -c $srcdir/lat.$i.gz |" "ark:|gzip -c >$destdir/lat.$i.gz" || exit 1 &
  done
  wait
fi

echo "$0: Copy lattices to $destdir DONE!"
exit 0
